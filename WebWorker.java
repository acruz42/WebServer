...
public void run()
{
    ...
    try {
        ...
        String urlAddress = readHTTPRequest( is );
        System.out.println( urlAddress );
        writeHTTPHeader( os , "text/html" , urlAddress );
        if ( urlAddress.equals( null ) ) {
            os.write("<html><head></head><body>\n".getBytes());
            os.write("<h3>My web server works!</h3>\n".getBytes());
            os.write("</body></html>\n".getBytes());
        }
        else
            writeContent( os, urlAddress );
        os.flush();
        socket.close();
    } catch ( Exception e ) {
...
private void writeHTTP404ErrorHeader(OutputStream os, String contentType, String urlAddress) throws Exception
{
    Date d = new Date();
    DateFormat df = DateFormat.getDateTimeInstance();
    df.setTimeZone( TimeZone.getTimeZone( "GMT" ) );
...
* @param os is the OutputStream object to write to
**/
private void writeContent(OutputStream os, String urlAddress) throws Exception
{
    Date d = new Date();
	DateFormat df = DateFormat.getDateTimeInstance();
	df.setTimeZone(TimeZone.getTimeZone( "GMT" ));
	...
	try{
		...
		while( ( dataContent = inBuffer.readLine() ) != null ) {
            ...
            if ( dataContent.contains( "<cs371server>" ) )
				os.write( "Adrian's Server ID string\n".getBytes() );
		}
		
	} catch(FileNotFoundException e) {
		writeHTTP404ErrorHeader( os , "text/html" , urlAddress );
	}
}
